...
Production fan control currently supports only the promoted First Series Expert 1.3K-FA model and SET-menu topology. Unsupported models are blocked before SET. The captured Expert 1.5K-FA Second Series CONFIG-first path is available only through the guarded Menu Debug reversible test until its complete apply/restore report is verified; F-KFA NORMAL/QUIET layouts remain topology-only.
...
Use `GET /api/v1/fan-policy` for the disabled-by-default temperature/hysteresis decision and navigation receipt. Production control requires a promoted model profile and currently supports the verified First Series Expert 1.3K-FA path; unsupported models are blocked before SET. Active control requires polling mode `both` and starts only after fresh protocol and checksum-valid LCD RX evidence. The server verifies the model plus first SET-menu topology and then every waypoint. If initially OPERATE, it temporarily requests and verifies STANDBY because tested hardware ignores SET on the OPERATE home screen. After verified SAVE and return home it restores and verifies OPERATE only when this transaction originally disabled it.
