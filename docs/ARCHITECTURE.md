...
- **Wake reserves actuation without holding the coordinator lock across serial I/O.** Manual writes, menu/fan leases, and a second wake fail busy while the reservation is active. Overtemperature safety waits for wake to finish before preempting. Owner names are diagnostics only: every successful acquisition returns an opaque lease identity, and only that exact lease can release the current reservation.
