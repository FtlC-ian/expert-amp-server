...
- **Third Series 2K-FA production control is narrower than First/Second Series.** It is bound to `EXPERT 2K-FA`, the exact Third Series topology, and an explicit operator-declared firmware value of `Rel.26_03_24_A` because status polling does not expose firmware. It maps hardware QUIET to logical Normal cooling and hardware NORMAL to logical high cooling, starts only from verified STANDBY/RX, and never sends DISPLAY or an OPERATE/STANDBY command. Its exact 23-state reversible D1 report is retained as regression evidence. Menu Debug remains the guarded evidence path for other firmware or topology combinations.
