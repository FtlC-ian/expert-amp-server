...
Menu Debug & Reporting is a separate disabled-by-default discovery boundary. Arming requires the exact acknowledgement, fresh protocol STANDBY/RX, a recent checksum-valid STANDBY home display, inactive fan control, disarmed overtemperature standby, and an exclusive actuation lease. Its token is memory-only and short-lived; each mutation consumes the latest revision so stale tabs cannot replay commands. Each accepted advance sends at most one reviewed SET, RIGHT, or model-specific no-save exit and then waits for a newer checksum-valid screen. Protocol status, LCD evidence, discovery and planned authorizations are bound to the observed amplifier model and the serial-session generation that produced them. The live transport atomically requires that same model, STANDBY, and known RX under its write lock before sending. Opening a replacement port immediately invalidates the entire active session and discards accumulated report evidence, preventing replacement hardware from continuing or relabeling prior evidence; the transport also captures the verified port under its write lock so a reconnect cannot redirect a command.
