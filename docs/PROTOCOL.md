...
Reviewed reversible fan-test profiles cover the First Series Expert 1.3K-FA and the CONFIG-first Expert 1.5K-FA Second Series topology. Both profiles have physically verified apply and restoration evidence and are eligible for production fan control. One additional candidate-only grammar covers the exact Expert 2K-FA Third Series model/topology/firmware combination for a single STANDBY/RX NORMAL→QUIET→NORMAL evidence run; it is not a production fan-policy profile. Unknown models and F-KFA NORMAL/QUIET layouts remain topology-only; QUIET is never interpreted as high cooling.
