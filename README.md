...
The reporting wizard also recognizes the field-reported Third Series Expert 2K-FA bankless `CAT` home header, distinct setup grid, and `POWER-SUPPLY FAN` NORMAL/QUIET page. It accepts native SPE `Rel.` firmware strings such as `Rel.26_03_24_A`, and Second Series setup recognition accepts either input number. Third Series remains topology-only: it cannot authorize a value change, SAVE, or DISPLAY recovery.
